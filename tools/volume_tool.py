...
def get_most_active_by_dollar_volume(
    count: int = 20,
) -> tuple[list[str], dict[str, float]]:
    """
    (상위 티커 목록, 티커별 거래대금) 을 반환합니다.

    Yahoo most_actives 스크리너에서 후보를 받아 시총 구간으로 거른 뒤
    거래대금(3개월 평균 거래량 × 현재가)으로 재정렬합니다.
    순위가 아니라 거래대금 원본값을 함께 넘기는 이유는, 순위만 쓰면
    1위가 2위의 2배여도 점수 차가 5%밖에 나지 않기 때문입니다.
    ...
    return ranked, {t: dollar_volumes[t] for t in ranked}
